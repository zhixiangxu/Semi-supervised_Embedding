...
layer_sizes = [dconf['data_shape'], 1000, 500, 250, 250, 250, dconf['nb_class']]

L = len(layer_sizes) - 1  # number of layers

#num_examples = dconf['dataset_shape'][0] * dconf['dataset_shape'][1]
num_epochs = 25
num_labeled = len(dset.train['xl'])
num_examples = num_labeled * num_epochs
...
batch_size = 100
num_iter = int(num_examples/batch_size)  # number of loop iterations
# num_iter = 500
...
def bi(inits, size, name):
    return tf.Variable(inits * tf.zeros([size]), name=name)
...
def encoder(inputs, noise_std):
    ...
    for l in range(1, L+1):
        ...
        if l == L:
            ...
            #h = tf.nn.sigmoid(z + weights["beta"][l - 1])
        else:
...
loss = cost + alpha * u_cost  # total cost
...
# VERY IMPORTANT FOR houston dataset
...
print("=== Training ===")
test_x, test_y = dset.get_test(True)
...
pred = sess.run(tf.argmax(y, 1), feed_dict={inputs: test_x, outputs: test_y, training: False})
...
print("Final Accuracy: ", int_accuracy(pred, dset.test['y']))
print("Final Accuracy: ", sess.run(accuracy, feed_dict={inputs: test_x, outputs: test_y, training: False}), "%")
